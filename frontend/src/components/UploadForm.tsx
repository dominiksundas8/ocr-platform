...
/**
 * 📤 Upload Form (Atomatic Client Component)
 * Gestisce il drag-and-drop, la selezione dei file e l'invio alla Server Action.
 */
export default function UploadForm() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      ...
      // Generazione preview locale (non inviata al server, solo per UI)
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 🎯 Redirect istantaneo alla pagina di dettaglio
      router.push(`/dashboard/documents/${result.data.document.id}`);
    } catch (error: any) {
